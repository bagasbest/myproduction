...
class MaterialAddEditActivity : AppCompatActivity() {

    private var binding: ActivityMaterialAddBinding? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityMaterialAddBinding.inflate(layoutInflater)
        setContentView(binding?.root)

        val option = intent.getStringExtra(OPTION)
        showDropdownSize()
        if (option == "add") {
            binding?.title?.text = "Bahan Baku Baru"
            binding?.saveBtn?.text = "Tambahkan Bahan Baku"
        } else {
            binding?.title?.text = "Edit Bahan Baku"
